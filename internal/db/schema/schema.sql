...
CREATE TABLE facilities (
    ...
    reminder_hours_before INTEGER NOT NULL DEFAULT 24 CHECK (reminder_hours_before > 0),
    tier_booking_enabled BOOLEAN NOT NULL DEFAULT 0,
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX idx_clinic_types_facility_id ON clinic_types(facility_id);
...
CREATE INDEX idx_clinic_sessions_clinic_type_id ON clinic_sessions(clinic_type_id);
CREATE INDEX idx_clinic_sessions_pro_id ON clinic_sessions(pro_id);
CREATE INDEX idx_clinic_sessions_start_time ON clinic_sessions(start_time);

------ CLINIC ENROLLMENTS ------
CREATE TABLE clinic_enrollments (
    id INTEGER PRIMARY KEY,
...
CREATE INDEX idx_clinic_enrollments_session_id ON clinic_enrollments(clinic_session_id);
CREATE INDEX idx_clinic_enrollments_user_id ON clinic_enrollments(user_id);
CREATE INDEX idx_clinic_enrollments_status ON clinic_enrollments(status);
